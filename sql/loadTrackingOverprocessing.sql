...
where to_date(booked_datetime) >= {startdate}
group by 1
)
...
,loads_filter as
(
select lb.load_num, count(distinct seq_num) num_seq_num, count(distinct carrier_code) num_carriers
from nast_carrier_domain.broker.load_books lb
...
select
    ...
    and lb.carrier_branch_code != '0273'
    and to_date(version_start_datetime_tz) >= {startdate}
)
...
inner join loads_filter lf on lf.load_num = a.load_num
where /*check_call_type = 'CC' and*/ to_date(entered_datetime_tz) >= {startdate}
